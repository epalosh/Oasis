...
def sendNotif():
    ...
    # Optional: Read and print the updated value
    updated_doc = counter_ref.get()
    if updated_doc.exists:
        print(f"Updated fall count: {updated_doc.to_dict()['count']}")
    else:
...
while True:
                ...
                if current_time - prev_time > 0.25:
                    ...
                    if prev_midpoint_x is not None and prev_midpoint_y is not None:
                        ...
                        stage_1 = midpoint_fall_y > 30
                                ...
                                # # if not (time.time() - fall_detected_time < 3):
                                # sendNotif()
    ...
    title_text = "OASIS Fall Detection v2.1"
    font_scale = 1.5
    thickness = 2
    font = cv2.FONT_HERSHEY_SIMPLEX
