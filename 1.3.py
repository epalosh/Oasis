...
def draw_dashboard(frame, values_dict, stage_1, stage_2, stage_3):
    dashboard_width = 360
    ...
    stage_width = bar_width // 3

    stages = [(stage_1, "Stage 1"), (stage_2, "Stage 2"), (stage_3, "Stage 3")]
...
while True:
    ...
    if keypoints is not None:
        ...
        frame = results[0].plot(boxes=True, masks=True, probs=False, labels=False, line_width=4, font_size=1)

    title_text = "OASIS Fall Detection v1.3"
    font_scale = 1.5
    thickness = 2
    font = cv2.FONT_HERSHEY_SIMPLEX
    ...
    if stage_3 and (time.time() - fall_detected_time < 3):
    ...
    frame_with_dashboard = draw_dashboard(frame, dashboard_data, stage_1, stage_2, stage_3)

    cv2.imshow("YOLOv8 Pose Detection + Dashboard", frame_with_dashboard)
