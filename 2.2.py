...
class FallDetector:
    ...
    def draw_dashboard(self, frame, values):
        """Dashboard drawing logic beneath the video frame."""
        # Make dash the same width as frame, but a fixed height:
        dash_h = 750  # height in pixels
        dash_color = (30, 30, 30)
        ...
        # Create a bar-height dashboard
        dash = np.full((dash_h, frame.shape[1], 3), dash_color, dtype=np.uint8)

        # Stage bar (you can shrink or reposition it within dash_h)
        bar_h = 30
        bar_w = frame.shape[1] - 2*pad
        stage_w = bar_w // 4
        for i, (flag, label) in enumerate(
            [(self.stage1, 'Stage 1'), (self.stage2, 'Stage 2'),
             (self.stage3, 'Stage 3'), (self.fb_fall, 'F/B')]
        ):
            x0 = pad + i*stage_w
            ...
            cv2.putText(
                ...
                font,
                0.5,
                (0, 0, 0) if flag else (150, 150, 150),
                1,
                cv2.LINE_AA
            )

        # Draw the key/value pairs below the stage bar
        y = 20 + bar_h + 30
        for k, v in values.items():
            cv2.putText(dash, f"{k}: {v}", (pad, y), font, scale, text_color, 1, cv2.LINE_AA)
            y += lh

        # Stack video on top of dashboard
        return np.vstack((frame, dash))
    ...
    def annotate(self, frame, values):
        ...
        # Title
        title = "OASIS Fall Detection v2.2"
        fs, th = 1.5, 2
